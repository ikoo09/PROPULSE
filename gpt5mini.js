module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // ==========================
    // 1. CEK CACHE REDIS (CACHE 6 JAM UNTUK TREND, PERMANEN UNTUK SCRIPT)
    // ==========================
    let cacheKey = null;

    if ((feature === "trend" || feature === "script") && input && UPSTASH_URL && UPSTASH_TOKEN) {
      
      // Menggunakan Hash SHA-256 agar key unik, aman, dan konsisten
    ...
    if ((feature === "trend" || feature === "script") && cacheKey && text && UPSTASH_URL && UPSTASH_TOKEN) {
      ...
      // VALIDASI JSON: Hanya simpan ke cache jika AI memberikan format JSON yang valid
      // Ini mencegah cache menyimpan error ("Gagal memproses script")
      ...
      try {
          ...
          console.warn("Respons AI bukan JSON yang valid. Melewati penyimpanan Cache.");
      }
      ...
      if (isValidJson) {
          try {
            ...
            let redisCommand;
            if (feature === "trend") {
                // Cache 6 Jam untuk Trend
                redisCommand = ["SET", cacheKey, payloadToSave, "EX", 21600];
            } else {
                // Cache Permanen untuk Script (tanpa parameter EX)
